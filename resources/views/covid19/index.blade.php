<x-bootstrap-theme>
    ...
    <table class="table table-sm table-bordered text-end">
        ...
        <tr>
            <td>{{ $item->date }}</td>
            <td>{{ $item->country }}</td>
            <td>{{ number_format( $item->total ) }}</td> <!-- สัปดาห์ที่6 -->
            <td>{{ $item->total }}</td>
            <td>{{ $item->active }}</td>
            <td>{{ $item->death }}</td>
            <td>{{ $item->recovered }}</td>
            <td>{{ $item->total_in_1m }}</td>
            <td>{{ number_format( $item->total_in_1m , 2 ) }}</td> <!-- สัปดาห์ที่6 -->
            <td>
                <a href="{{ url('/covid19/'.$item->id ) }}" class="btn btn-sm btn-primary">View</a> <!-- สัปดาห์ที่6 -->
